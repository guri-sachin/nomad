...
function Admin(){
  ...
  const navigate = useNavigate();
   useEffect(()=>{
    axios.get("http://localhost:4200/listmessage").then(res=>setProducts(res.data));
},[]);
 ...
 function Piya(e){
  ...
  axios.get(`http://localhost:4200/messagedelete/${id}`).then(
...
function onEdit(e){
  const id = e.target.id;
  axios.get(`http://localhost:4200/messageedit/${id}`).then(
      res =>{
     var diojk =res.data 
