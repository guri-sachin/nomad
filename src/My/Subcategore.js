...
function Admin(){
  ...
  const [status, setStatus] = useState('');
   ...
   useEffect(()=>{
    axios.get("http://localhost:4200/subcategore").then(res=>setProducts(res.data));
},[product]);
...
function Show3(e){
  ...
  axios.post(`http://localhost:4200/subupdate/${id}`,data).then(
    res=> console.log(res))
    console.log(id);
  console.log(data);
  Swal.fire({
 ...
 function Piya(e){
  ...
  axios.get(`http://localhost:4200/subcatedelete/${id}`).then(
...
function onEdit(e){
  const id = e.target.id;
  axios.get(`http://localhost:4200/subediton/${id}`).then(
      res =>{
     var diojk =res.data 
